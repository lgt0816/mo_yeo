...
public class RcLookActivity extends AppCompatActivity {
    ...
    protected void onCreate( Bundle savedInstanceState) {
        ...
        rc_sub_title =intent.getString("rc_s_subtitle");
       ...
       // txt_rcname = (TextView) findViewById(R.id.txt_rcname);
        ...
        String[] array_rc = new String[8]; //title1,subtitle2,content3,rule4,id5,type6
        String ok = "SELECT04";
        Object rcvDataFromServer = null;
        try {
            rcvDataFromServer = new CommAPI().execute(ok, rc_title).get();
            array_rc = rcvDataFromServer.toString().split("ㅩ");
           // txt_rcname.setText(array_rc[1]);
            txt_rcid.setText(array_rc[5]);//rule
            txt_rccontext.setText(array_rc[3]);//content
            txt_rcrule.setText(array_rc[4]);//contnt
        }catch (Exception e) {
            e.printStackTrace(); }
