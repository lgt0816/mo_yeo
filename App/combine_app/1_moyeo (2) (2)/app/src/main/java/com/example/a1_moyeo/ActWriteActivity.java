    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            ...
            case R.id.toolbar_finish_button:{ //완료 버튼 눌렀을 때
                ...
                try {
                    rcvDataFromServer = new CommAPI().execute(ok,atlist_title).get();
                }catch (Exception e) {
                    e.printStackTrace();
                }
                ...
                ok = "INSERT08";
                try {
                   new CommAPI().execute(ok,edit_acttitle.getText().toString(),edit_actcontent.getText().toString(),rcvDataFromServer.toString()).get();
                }catch (Exception e) {
                    e.printStackTrace();
                }
                Toast.makeText(getApplicationContext(),"활동이 생성되었습니다.",Toast.LENGTH_LONG).show();
                finish();
                return true;
            }
        }
